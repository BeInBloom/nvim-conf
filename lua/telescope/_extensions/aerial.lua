...
local function iter_highlight_matches(query, root, bufnr)
  return query:iter_matches(root, bufnr, 0, -1)
end
...
local function aerial_picker(opts)
  ...
  local show_lines = opts.show_lines or conf.show_lines
  ...
  local displayer = opts.displayer or entry_display.create({
    separator = " ",
    items = layout,
  })
  ...
  local function collect_buf_highlights()
    ...
    local tree = parser:trees()[1] or parser:parse()[1]
    if not tree then
      return {}
    end
    ...
    if query then
      for _, captures in iter_highlight_matches(query, root, bufnr) do
  ...
  if show_columns == "lines" or show_columns == "both" then
    buf_highlights = collect_buf_highlights()
  end
